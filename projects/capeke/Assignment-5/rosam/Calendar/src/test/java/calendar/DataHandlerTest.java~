...
public class DataHandlerTest{
  ...
  public void test01()  throws Throwable  {
      ...
      badappt.setValid();
      DataHandler dh = new DataHandler();
      assertTrue(dh.isValid());
      assertTrue(dh.isAutoSave());
      dh.saveAppt(appt);
      assertEquals(dh.getNextApptOccurrence(appt, cal1), null);
      ...
      assertEquals(dh.getApptOccurences(appt2, cal2, cal1).size(), 0);
      assertEquals(dh.getApptOccurences(appt3, cal1, cal2).size(), 0);
      assertEquals(dh.getApptOccurences(appt2, cal1, cal2).size(), 1);
      assertEquals(dh.getApptOccurences(appt4, cal1, cal2).size(), 3);
